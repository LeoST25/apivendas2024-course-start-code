...
describe('InMemoryRepository unit tests', () => {
  ...
  describe('applyFilter', () => {
    it('should no filter items when filter is null', async () => {
      ...
      const result = await sut['_applyFilter'](items, null)
    ...
    it('should filter the data using filter param', async () => {
      ...
      const spyFilterMethod = jest.spyOn(items, 'filter' as any)
      let result = await sut['_applyFilter'](items, 'TEST')
      expect(spyFilterMethod).toHaveBeenCalledTimes(1)
      expect(result).toStrictEqual([items[0], items[1]])

      result = await sut['_applyFilter'](items, 'test')
      expect(spyFilterMethod).toHaveBeenCalledTimes(2)
      expect(result).toStrictEqual([items[0], items[1]])

      result = await sut['_applyFilter'](items, 'no-filter')
  ...
  describe('applySort', () => {
    it('should not sort items', async () => {
      ...
      let result = await sut['_applySort'](items, null, null)
      expect(result).toStrictEqual(items)

      result = await sut['_applySort'](items, 'id', 'asc')
      expect(result).toStrictEqual(items)
    })
    ...
    it('should sort items', async () => {
      ...
      let result = await sut['_applySort'](items, 'name', 'desc')
      expect(result).toStrictEqual([items[2], items[0], items[1]])

      result = await sut['_applySort'](items, 'name', 'asc')
  ...
  describe('applyPaginate', () => {
    it('should paginate items', async () => {
      ...
      let result = await sut['_applyPaginate'](items, 1, 2)
      expect(result).toStrictEqual([items[0], items[1]])

      result = await sut['_applyPaginate'](items, 2, 2)
      expect(result).toStrictEqual([items[2], items[3]])

      result = await sut['_applyPaginate'](items, 2, 3)
