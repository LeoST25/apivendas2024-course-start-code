  ...
  implements RepositoryInterface<Model, CreateProps>
{
  ...
  async search(props: SearchInput): Promise<SearchOutput<Model>> {
    ...
    const filteredItems = await this._applyFilter(this.items, filter)
    const orderedItems = await this._applySort(filteredItems, sort, sort_dir)
    const paginatedItems = await this._applyPaginate(
      orderedItems,
      page,
      per_page,
    )
  ...
  protected abstract _applyFilter(
    items: Model[],
    filter: string | null,
  ): Promise<Model[]>

  protected async _applySort(
    items: Model[],
    sort: string | null,
  ...
  protected async _applyPaginate(
    items: Model[],
    page: number,
    per_page: number,
  ): Promise<Model[]> {
