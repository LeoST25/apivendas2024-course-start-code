...
describe('ProductsInMemoryRepository unit tests', () => {
  ...
  describe('applyFilter', () => {
    it('should no filter items when filter is null', async () => {
      ...
      const result = await sut['_applyFilter'](sut.items, null)
    ...
    it('should filter the data using filter param', async () => {
      ...
      const spyFilterMethod = jest.spyOn(sut.items, 'filter' as any)
      const result = await sut['_applyFilter'](sut.items, 'TEST')
  ...
  describe('applySort', () => {
    it('should sort items by created_at when sort param is null', async () => {
      ...
      sut.items.push(...items)
      const result = await sut['_applySort'](sut.items, null, null)
      expect(result).toStrictEqual([items[2], items[1], items[0]])
    })

    it('should sort items by name fields', async () => {
      const items = [
      ...
      sut.items.push(...items)
      const result = await sut['_applySort'](sut.items, 'name', 'desc')
